...
class MissionEngine:
    ...
    def generate_search_pattern(
        ...
        if pattern == SearchPattern.PARALLEL_TRACK:
            ...
            bounds = GeoBounds(min_lat=sw[0], min_lon=sw[1], max_lat=ne[0], max_lon=ne[1])
    ...
    def pattern_to_wpml(
        ...
        for lat, lon in coords:
            ...
            waypoints.append(Waypoint(lat=lat, lon=lon, altitude=altitude, speed=speed, gimbal_pitch=gimbal_pitch, actions=actions))
        settings = MissionSettings(auto_flight_speed=speed, max_flight_speed=max(speed * 2, 15.0), finish_action=FinishAction.GO_HOME)
    ...
    async def create_and_dispatch(
        ...
        coords = self.generate_search_pattern(pattern, center_lat, center_lon, radius_m, spacing_m, altitude)
        wpml_xml = self.pattern_to_wpml(coords, altitude, speed)
        record = MissionRecord(
            mission_id=mission_id, gateway_sn=gateway_sn, flight_id=flight_id,
            pattern=pattern, wpml_xml=wpml_xml, waypoints=coords,
    ...
    async def _dispatch(self, record: MissionRecord) -> None:
        ...
        await self._mqtt.send_flighttask_prepare(record.gateway_sn, record.flight_id, file_url, file_md5)
    ...
    async def execute_mission(self, mission_id: str) -> None:
        ...
        if record.status not in (MissionStatus.READY, MissionStatus.PREPARING):
            raise RuntimeError(f"Mission {mission_id} cannot be executed in state {record.status}")
        record.status = MissionStatus.EXECUTING
        record.updated_at = time.time()
        await self._mqtt.send_flighttask_execute(record.gateway_sn, record.flight_id)
    ...
    async def resume_from_breakpoint(self, mission_id: str) -> None:
        ...
        if not remaining:
            ...
            record.updated_at = time.time()
            return
        record.wpml_xml = self.pattern_to_wpml(remaining)
        record.updated_at = time.time()
        await self._dispatch(record)
        logger.info("Mission %s resuming from waypoint %d", mission_id, record.breakpoint_index)

    async def _on_flighttask_progress(self, gateway_sn: str, payload: Dict[str, Any]) -> None:
    ...
    async def _on_services_reply(self, gateway_sn: str, payload: Dict[str, Any]) -> None:
    ...
    async def _auto_followup(self, completed: MissionRecord) -> None:
        try:
            ...
            logger.info("Auto-dispatching follow-up for mission %s", completed.mission_id)
            await self.create_and_dispatch(
                gateway_sn=completed.gateway_sn, pattern=SearchPattern.PARALLEL_TRACK,
                center_lat=center_lat, center_lon=center_lon, radius_m=200.0,
                spacing_m=25.0, altitude=40.0, speed=3.0, auto_followup=False,
            )
        except Exception:
            logger.exception("Auto follow-up failed for mission %s", completed.mission_id)
